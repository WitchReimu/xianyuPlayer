...
public class WheelView extends View implements Runnable {
    ...
    public void setLrcData(List<LrcBean> newData) {
        setData(newData, 0);
    }
    ...
    private void computeIndicatorRect() {
        ...
        rectIndicatorHead.set(rectDrawn.left, indicatorHeadCenterYCoordinate - halfIndicatorSize, rectDrawn.right, indicatorHeadCenterYCoordinate + halfIndicatorSize);
    ...
    public String formatItem(Object item) {
        ...
        if (item instanceof LrcBean) {
            return ((LrcBean) item).lrc;
        }
        if (formatter != null) {
    ...
    private Rect timeBound = new Rect();
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        Object item = data.get(currentPosition);
        if (item instanceof LrcBean) {
            long time = ((LrcBean) item).time;
            long minutes = TimeUnit.MILLISECONDS.toMinutes(time);
            long second = TimeUnit.MILLISECONDS.toSeconds(time) - TimeUnit.MINUTES.toSeconds(minutes);
            String timeStr = String.format(Locale.getDefault(), "%02d:%02d", minutes, second);
            paint.getTextBounds(timeStr, 0, timeStr.length(), timeBound);
            float timeIndicatorWidth = paint.measureText(timeStr);
            canvas.drawText(timeStr, getWidth() - timeBound.width() / 2, rectIndicatorFoot.bottom + timeBound.height() / 2, paint);
        }
    ...
    private void handleActionMove(MotionEvent event) {
        ...
        //
        currentPosition = computePosition(getItemCount());
